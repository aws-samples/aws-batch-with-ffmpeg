...
def delete_bucket_with_retry(bucket_name, max_retries=5, delay=5):
    for attempt in range(max_retries):
        try:
            ...
            if e.response["Error"]["Code"] == "BucketNotEmpty":
                print(f"Bucket not empty, retrying in {delay} seconds...")
                time.sleep(delay)
            else:
                # If it's a different error, raise it
                raise

    print(f"Failed to delete bucket after {max_retries} attempts")
    ...
    try:
        ...
        for bucket in buckets:
            ...
            if fnmatch.fnmatch(bucket_name, bucket_pattern):
                logger.info(f"Deleting bucket content : {bucket_name}")
                delete_bucket_contents(bucket_name)
                logger.info(f"Deleting bucket : {bucket_name}")
                delete_bucket_with_retry(bucket_name=bucket_name)
